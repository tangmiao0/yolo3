...
# a = get_batch_data([c], 20, [416, 416], args.anchors, 'train')
...
train_dataset = train_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         inp=[x, args.class_num, args.img_size, args.anchors, True, True, False],
...
val_dataset = val_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         inp=[x, args.class_num, args.img_size, args.anchors, False, False, args.letterbox_resize, 'val'],
                         Tout=[tf.int64, tf.float32, tf.float32, tf.float32, tf.float32]),
    num_parallel_calls=args.num_threads
)
...
# get an element from the chosen dataset iterator 这里的y_true 中的boxes都是在原图的坐标
